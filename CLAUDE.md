...
images/
  ...
  Asortyment/               — brand/product gallery source photos (promo banner images, client-supplied)
    ...
    Alkohol1.jpg            — "АЛКОГОЛЬ В ПРОДАЖІ" promo graphic (Martini, Morosha, Sinevir vodka),
                               ~1:1 (774×768). Used in the Gallery grid (#gallery), NOT the brand
                               gallery in Assortment — despite living in this folder.
...
4. **Assortment** (`#assortment`) — 5 category cards (fish, meat, drinks, sweets, preserves) + illustrated menu image + 9-card brand/product gallery grid (`.prod-card`, lightbox-enabled via `openLb()`, captions translated via `T.*.assort.brands`). The drinks card (`c-drink-d` / `T.*.assort.drink.d`) now mentions alcohol and energy drinks alongside water/juice/soda, added when that assortment expanded.
5. **Gallery** (`#gallery`) — one unified masonry grid, `grid grid-cols-2 md:grid-cols-3`: original 4 curated photos (exterior/customers/closeup, `RAW/`) + a wide "shelf aisle" establishing banner + 17 real product/case-closeup photos (both `images/Interior/` batches + `images/Asortyment/Alkohol1.jpg`, the alcohol/energy-drinks promo added when that assortment expanded) + a `col-span-1` "coming soon" teaser card (`.gal-teaser`, soft green→cream→gold gradient, clock icon, `id="g-teaser"`, translated via `T.*.gallery.teaser`) that exactly fills the trailing grid gap after the last photo + a wide "full fridge" closing banner (23 photos total). Everything but the teaser card uses `.gal-wrap`/`.gal-img`/`.gal-zoom-icon` hover + `.reveal` scroll-animation, lightbox-enabled via `openLb()`; the teaser card is deliberately not clickable (no `onclick`/lightbox — it's not a photo) and uses its own `.gal-teaser:hover` lift instead. ⚠️ If the photo count in this grid ever changes, recheck whether the teaser's `col-span-1` still exactly closes the gap before the closing banner (it was `col-span-2` when there were 16 single-cell photos instead of 17 — verify with real `getBoundingClientRect()` measurements at both the `md:grid-cols-3` and mobile `grid-cols-2` breakpoints, not by eyeballing it, since the two breakpoints need different spans in general and only happened to both want `col-span-1` this time). (A separate "Nasze wnętrze" section briefly existed for the second photo batch; client asked to merge it back into Gallery, so there is currently only one gallery section on the page.)
